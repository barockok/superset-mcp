...
export function registerChartTools(server, client) {
    server.tool("list_charts", "List all charts in Superset. Returns chart names, types, and IDs.", {
    ...
    server.tool("get_chart", "Get detailed information about a specific chart.", { id: z.number().describe("Chart ID") }, async ({ id }) => toolHandler(async () => {
        const data = await client.get(`/api/v1/chart/${id}`);
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }));
    server.tool("get_chart_data", "Fetch the underlying data for a chart by its ID.", { id: z.number().describe("Chart ID") }, async ({ id }) => toolHandler(async () => {
