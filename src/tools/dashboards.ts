...
export function registerDashboardTools(server: McpServer, client: SupersetClient) {
  server.tool(
    "list_dashboards",
    "List all dashboards in Superset. Returns dashboard titles, IDs, and status.",
    {
  ...
  server.tool(
    "get_dashboard",
    "Get detailed information about a specific dashboard by ID or slug.",
    { id_or_slug: z.string().describe("Dashboard ID or slug") },
    async ({ id_or_slug }) => toolHandler(async () => {
  ...
  server.tool(
    ...
    "Get all charts contained in a dashboard.",
    { id_or_slug: z.string().describe("Dashboard ID or slug") },
    async ({ id_or_slug }) => toolHandler(async () => {
  ...
  server.tool(
    "export_dashboard",
    "Export dashboards as YAML/JSON bundle.",
    { ids: z.array(z.number()).describe("Array of dashboard IDs to export") },
    async ({ ids }) => toolHandler(async () => {
